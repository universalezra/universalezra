class BioVAuth{
    constructor(){
        this.env={WORKER_URL:'https://auth-logs.universalezra.workers.dev/',RATE_LIMIT_WINDOW:300000,RATE_LIMIT_MAX_ATTEMPTS:5};
        this.elements={
        ...
        this.securityConfig={
            ...
            loginStatusCookieName:'isEzraLoggedIn', // Nama cookie untuk status login
            loginCookieExpiryMinutes:10 // Waktu kedaluwarsa untuk cookie login (10 menit)
        };
    ...
    async handleLogin(){
        ...
        try{
            ...
            if(responseData.authenticated){
                // Menggunakan cookie untuk status login dengan waktu kedaluwarsa
                ...
                window.location.href='dashboard/elvi.html';
            }else{
    ...
    initiateWebAuthn(){
        ...
        this.updateStatus('Initiat biometric authentication...','info');
        setTimeout(()=>this.updateStatus('use your device biometric authenticator','info'),1000);
    }

    // --- Metode Pembantu untuk Cookie ---
    setCookie(name, value, minutes) {
        const d = new Date();
        d.setTime(d.getTime() + (minutes * 60 * 1000)); // Konversi menit ke milidetik
        const expires = "expires=" + d.toUTCString();
        document.cookie = name + "=" + value + ";" + expires + ";path=/;SameSite=Strict;Secure";
    }

    // Metode ini mungkin sudah ada di SecuritySystem, tetapi ditambahkan di BioVAuth
    // jika Anda ingin BioVAuth juga memiliki kemampuan membaca cookie ini secara langsung.
    // Namun, dalam konteks ini, getCookie di SecuritySystem sudah cukup untuk bioVAuthSecurity.
    // Jika Anda ingin mengecek status login dari cookie isEzraLoggedIn di BioVAuth:
    getCookie(name) {
        const nameEQ = name + "=";
